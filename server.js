...
var server = net.createServer(function(socket){
    ...
    socket.on('data', function(data){
        ...
        if(im == ''){
            ...
            if(myCRC16(buf.slice(8, buf.length-4)) != buf.toString('hex', buf.length-2, buf.length)){
                console.log('CRC difference');
                return;
            }
            console.log("length records: " + length_records);
            console.log("count record: " + count_record);
            console.log('CRC tracker: ' + buf.toString('hex', buf.length-2, buf.length) + '<>' + myCRC16(buf.slice(8, buf.length-4)) + ' My CRC');
            var res = buf.slice(9,10);
            socket.write('\x00' + '\x00' + '\x00' + res);
            for(var i = 0; i < count_record; i++, all_records++){
                data_socket[all_records] = buf.slice(10  + (i*length_rec), 10 + length_rec + (i*length_rec));
            }
        }
    });
    ...
    socket.on('close', function(){
        console.log('IMEI: ' + im);
        for(var i = 0; i < all_records; i++) {
            process(data_socket[all_records - 1]);
        }
...
function process(buf){
    var unix_time = parseInt(buf.toString('hex', 0, 8), 16);
    var timestamp = new moment(unix_time).format('MMMM Do YYYY, H:mm:ss');
    longitude = buf.readUInt32BE(9)/10000000;
    latitude = buf.readUInt32BE(13)/10000000;
    altitude = buf.readUInt16BE(17);
    sputnik = buf.readUInt8(21);
    speed = buf.readUInt16BE(22);
    console.log(unix_time.toString('10'));
